---
title: 'Modèle de probabilité de transmission : distance 1m'
author: "Manal NHILI"
output:
  html_document:
    theme: paper
...
```{r}
sero <- read_excel("data_seg_1m.xlsx")
...
```
```{r}
#save(ppr_mcmc, inits, file = "p_samples_1m.RData")
```

```{r}
load("p_samples_1m.RData")
```
...
An ESS of 103073.4 suggests good mixing and provides a reasonable estimate of the effective information content in our MCMC samples for the variable **`p`**.
