...
```{r }
df <- read_excel("D:/cluster_model/bernoulli_data_seuil_0.3_cutt_off_2metres.xlsx")
str(df)
...
```{r}
df$ID <- paste(df$experiment, df$contact_duration, df$collar_tag, sep = "_") 

```


```{r}
lst <- lapply(split(df, df$ID), function(x) {
  list(
    time = x$time,
    distance = x$distance   
    #y = x$infected
...
```{r}
extractTime <- nimbleFunction(
  run = function(lst, i) {
    returnType(double(1))
    return(as.integer(lst[[i]]$time))
  }
)
```
...
```{r}
extractDistance <- nimbleFunction(
  run = function(lst, i) {
    returnType(double(1))
    return(as.numeric(lst[[i]]$distance))
  }
)
