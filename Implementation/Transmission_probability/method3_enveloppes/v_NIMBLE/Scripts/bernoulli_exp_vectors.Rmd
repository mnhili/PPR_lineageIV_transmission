...
library(tidyr)
library(dplyr)
library(parallel) # to run chains in parallele
```
...
ppr_code <- nimbleCode({
  ...
  probinsuc <- numeric(length(t))
  probind <- numeric(length(env))
  
  dynamicIndexingFunc <- nimbleFunction(environment(), function(counter, fine) {
    probinsuc[counter:fine]
  })
  
  for(e in env){
    fine <- counter + e - 1
    probinsucind <- dynamicIndexingFunc(counter, fine)
