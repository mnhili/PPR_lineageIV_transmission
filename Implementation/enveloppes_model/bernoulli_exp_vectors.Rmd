...
```{r }
df <- read_excel("bernoulli_data_seuil_0.3_cutt_off_2metres.xlsx")
str(df)
...
d <- df$distance
env <- replicate(34, 3)
begin<-seq(1,3*34+1,by=3)[1:34]
end<-seq(3,3*(34+1),by=3)[1:34]
```
...
ppr_code <- nimbleCode({
  ...
  for(e in env){
    fine <- counter + e - 1
    probinsucind <- c()
    for(k in counter:fine){
      probinsucind <- c(probinsucind,probinsuc[k])
    }
...
for(counter in 1:length(begin)){
  
  e<-end[counter]
  b<-begin[counter]
  probinsucind <-c()
  for(k in b:e){
    probinsucind <- c(probinsucin,probinsuc[k])
  }
...
data = list(y = y, d=d, t=t, env = env,begin=begin,end=end)
...
```{r}
ppr_mcmc <- nimbleMCMC(code = ppr_code2, 
                       inits = theta_inits,
                       data = data,
                       nburnin = 1000, 
